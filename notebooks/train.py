...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        last = Path(w / 'last.pt')  # убедись, что `w` — путь к директории weights
        best = Path(w / 'best.pt')
        ...
        for f in [last, best]:
            if f.exists():
                ...
                print(f"WARNING: File {f} not found, skipping strip_optimizer.")

        # Upload and log to wandb
        if opt.bucket:
            os.system(f'gsutil cp {final} gs://{opt.bucket}/weights')

        if wandb_logger.wandb and not opt.evolve:
            local_dir = '/content/temp_weights'
            os.makedirs(local_dir, exist_ok=True)

            local_model_path = os.path.join(local_dir, 'best.pt')
            shutil.copyfile(str(final), local_model_path)

            wandb_logger.wandb.log_artifact(local_model_path, type='model',
